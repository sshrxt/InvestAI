...
const InvestmentPage = () => {
  ...
  useEffect(() => {
    if(formData) { const converted = convertData(formData); 
  ...
  return (
    <div className="p-2">
      <div className="h-screen grid grid-cols-9 grid-rows-9 gap-3 overflow-hidden">
        ...
        <div className="bg-black-200 row-start-1 row-end-3"></div>
        <div className="bg-black-200 row-start-3 row-end-6"></div>
...
function convertData(formData: FormDataType): ConvertedDataType[] {
  ...
  for (let i = 0; i <= parseFloat(formData.growthYears); i += 0.5) {
    let approxValue = calculateFutureValue(deposit, rate / 100, compoundType, i);
...
function calculateFutureValue(deposit: number, annualRate: number, compound: number, years: number) {
  const monthlyRate = annualRate / compound;
  const totalPeriods = years * compound;
  const futureValuePrincipal = deposit * Math.pow(1 + monthlyRate, totalPeriods);
